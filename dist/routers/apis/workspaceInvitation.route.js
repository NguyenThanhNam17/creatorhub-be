...
class WorkspaceInvitationRoute extends BaseRoute {
    ...
    customRouting() {
        this.router.post("/inviteUserToWorkspace", this.authentication, this.route(this.inviteUserToWorkspace));
        this.router.get("/getMyWorkspaceInvitations", this.authentication, this.route(this.getMyWorkspaceInvitations));
    ...
    async inviteUserToWorkspace(req, res) {
        const { workspaceId, invitedUserId } = req.body;
        if (!workspaceId || !invitedUserId) {
            throw ErrorHelper.requestDataInvalid("workspaceId và invitedUserId là bắt buộc");
        }
    ...
    async getMyWorkspaceInvitations(req, res) {
    ...
    async acceptWorkspaceInvitation(req, res) {
    ...
    async rejectWorkspaceInvitation(req, res) {
        const { id: invitationId } = req.params;
        let invitation = await WorkspaceInvitationModel.findById(invitationId);
        if (!invitation) {
            throw ErrorHelper.forbidden("Lời mời không tồn tại");
    ...
    async cancelWorkspaceInvitation(req, res) {
        const { id: invitationId } = req.params;
        let invitation = await WorkspaceInvitationModel.findById(invitationId);
        if (!invitation) {
            throw ErrorHelper.forbidden("Lời mời không tồn tại");
