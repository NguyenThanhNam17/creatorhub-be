...
class WorkspaceRoute extends BaseRoute {
  ...
  customRouting() {
    this.router.post(
      "/archiveWorkspace",
      this.authentication,
      this.route(this.archiveWorkspace),
    );
    this.router.post(
      "/createWorkspace",
      this.authentication,
      this.route(this.createWorkspace),
    );
    this.router.get(
      "/listWorkspace",
      this.authentication,
    ...
    this.router.put(
      "/updateWorkspace",
      this.authentication,
      this.route(this.updateWorkspace),
    );
    this.router.post(
      "/restoreWorkspace",
      this.authentication,
      this.route(this.restoreWorkspace),
    );
    this.router.delete(
      "/deleteWorkspace",
      this.authentication,
      this.route(this.deleteWorkspace),
    );
  }
  ...
  async archiveWorkspace(req: Request, res: Response) {
    ...
    if (workspace.isDefault) {
      throw ErrorHelper.requestDataInvalid(
        "Không thể lưu trữ (archive) workspace mặc định.",
      );
    }
    ...
    console.log(
      `[WORKSPACE ARCHIVE] User ${user.username} (ID: ${user._id}) archived workspace ${workspace.name} (ID: ${workspace._id}) at ${new Date().toISOString()}`,
    );
  ...
  async createWorkspace(req: Request, res: Response) {
    ...
    if (existingWorkspace) {
      throw ErrorHelper.requestDataInvalid(
        "Tên Workspace đã tồn tại cho người dùng này",
      );
    }
  ...
  async getListWorkspace(req: Request, res: Response) {
    ...
    const sort = { createdAt: sortDirection } as any;
  ...
  async detailWorkspace(req: Request, res: Response) {
    ...
    // Lấy thông tin Owner
    const owner = await UserModel.findById(workspace.ownerId).select(
      "-password -key",
    );
  ...
  async updateWorkspace(req: Request, res: Response) {
    ...
    const workspace: any = await WorkspaceModel.findById(workspaceId);
  ...
  async restoreWorkspace(req: Request, res: Response) {
    const { workspaceId } = req.body;
  ...
  async deleteWorkspace(req: Request, res: Response) {
    ...
    if (workspace.isActive) {
      throw ErrorHelper.requestDataInvalid(
        "Chỉ được xóa Workspace đã Archive.",
      );
    }
