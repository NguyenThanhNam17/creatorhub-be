...
class WorkspaceMemberRoute extends BaseRoute {
  ...
  customRouting() {
    this.router.post(
      ...
      this.authentication,
      this.route(this.createWorkspaceMember),
    );
    this.router.get(
      "/getAllWorkspaceMemberForAdmin",
      this.authentication,
      this.route(this.getAllWorkspaceMemberForAdmin),
    );
    this.router.get(
      "/getMyWorkspaces",
      this.authentication,
      this.route(this.getMyWorkspaces),
    );
    this.router.get(
      "/getOneWorkspaceMember/:id",
      this.authentication,
      this.route(this.getOneWorkspaceMember),
    );
    this.router.get(
      "/getAllUserWorkspaceMemberForAdmin/:id",
      this.authentication,
      this.route(this.getAllUserWorkspaceMemberForAdmin),
    );
    this.router.get(
      "/leaveWorkspace/:id",
      this.authentication,
      this.route(this.leaveWorkspace),
    );
    this.router.get(
      "/transferWorkspaceOwner/:id",
      this.authentication,
      this.route(this.transferWorkspaceOwner),
    );
    this.router.get(
      "/workspaceMembers/:workspaceId",
      this.authentication,
      this.route(this.getWorkspaceMembers),
    );
    this.router.post(
      "/removeWorkspaceMember/:id",
      this.authentication,
      this.route(this.removeWorkspaceMember),
    );
    this.router.post(
      "/updateWorkspaceMemberRole/:id",
      this.authentication,
      this.route(this.updateWorkspaceMemberRole),
    );
  }
  ...
  async createWorkspaceMember(req: Request, res: Response) {
    ...
    if (!currentMember) {
      throw ErrorHelper.permissionDeny(
        "Chỉ chủ sở hữu mới có thể thêm thành viên",
      );
    }
  ...
  async getAllWorkspaceMemberForAdmin(
    req: Request,
    res: Response,
    next: NextFunction,
  ) {
  ...
  async getMyWorkspaces(req: Request, res: Response, next: NextFunction) {
    const currentUserId = req.tokenInfo!._id;
  ...
  async getOneWorkspaceMember(req: Request, res: Response, next: NextFunction) {
    const { id } = req.params;
  ...
  async getAllUserWorkspaceMemberForAdmin(req: Request, res: Response) {
    const { id } = req.params;
  ...
  async leaveWorkspace(req: Request, res: Response) {
    ...
    if (workspaceMember.role === "OWNER") {
      throw ErrorHelper.permissionDeny(
        "Chủ sở hữu không thể rời khỏi workspace",
      );
    }
  ...
  async transferWorkspaceOwner(req: Request, res: Response) {
    const currentUserId = req.tokenInfo!._id;
    const { id } = req.params;
    ...
    if (currentOwnerMember.role !== "OWNER") {
      throw ErrorHelper.permissionDeny(
        "Chỉ chủ sở hữu mới có thể chuyển quyền sở hữu",
      );
    }
    ...
    const workspace = await WorkspaceModel.findById(
      currentOwnerMember.workspaceId,
    );
    if (!workspace) {
      throw ErrorHelper.recoredNotFound("Workspace");
    }
